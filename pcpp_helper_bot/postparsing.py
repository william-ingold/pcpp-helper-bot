...
# \| -> Table cell divider
bad_pipe_pat = r'\\\|'
# Accidental newlines between the table
bad_newlines_pat = r'(Type|Item|Price\n\n)|(:----|:----|:----\n\n)|(\n\n\| Generated by)'
...
bad_linke_re = re.compile(bad_link_pat)
bad_pipe_re = re.compile(bad_pipe_pat)
bad_newlines_re = re.compile(bad_newlines_pat)
...
def detect_escaped_markdown(markdown):
    """Detects any escaped markdown from a PCPP table.
    
    Args:
        markdown (str): Markdown of a submission.
    
    Returns:
        True if escaped markdown detected, False otherwise.
    """
    
    # Search markdown for these bad markdown patterns
    bad_link = bad_linke_re.search(markdown)
    bad_pipes = bad_pipe_re.findall(markdown)
    bad_newlines = bad_newlines_re.search(markdown)
    
    return bad_link is not None or len(bad_pipes) >= 2 or bad_newlines is not None
...
def combine_iden_anon_urls(anon_urls: list, iden_urls: list, pcpp_parser: PCPPParser):
...
def count_table_types(tables):
    """Counts the types of tables found."""
...
def parse_submission(submission_html: str, submission_markdown: str, pcpp_parser: PCPPParser):
    """Parses a submission for PCPP elements, such as tables and urls.
    
    Args:
        submission_html (str): HTML of a submission.
        submission_markdown (str): Markdown of a submission.
        pcpp_parser (PCPPParser): PCPPParser object for resolving identifiable
                                    urls.
                                    
    Returns:
        (URLS without tables, List of (identifiable, anonymous) urls, and
        {'total', 'valid', 'invalid', 'bad_markdown'} dict of table data).
    """
